...
def predict_piece_MNIST(model,img_path):
    ...
    #predict sim vector
    predict= model.predict(np.array([img]), batch_size=None, verbose=1)[0]
    label=label_names[np.argmax(predict)]
    return label

def predict_piece(model,img_path):
    label_names=['dresses','pants','skirts','blouses','sweaters','shorts']
    ...
    # show_images([img])
    #predict sim vector
    predict= model.predict(np.array([img]), batch_size=None, verbose=0)[0]
    label=label_names[np.argmax(predict)]


    piece_color=compute_piece_color(col_img)
    show_images([bgr2rgb(col_img)],titles=[piece_color+'-'+label])
    # print(piece_color)
    with open('C:\\Users\\mosama\\PycharmProjects\\GP\\col_temp.pkl', 'wb') as output:
        pickle.dump(piece_color, output, pickle.HIGHEST_PROTOCOL)
    # with open('C:\\Users\\mosama\\PycharmProjects\\GP\\sds_temp.pkl', 'wb') as output:
    #     pickle.dump(sds, output, pickle.HIGHEST_PROTOCOL)
    return label




# name='Choker Neck Grid Print Top'
# query_path='C:\\Users\\mosama\\PycharmProjects\\GP\\crawled_images\\Shein Images\\Shein Crawled Blouses\\'+name+'.jpg'

query_path=sys.argv[1]
model_path='C:\\Users\\mosama\\PycharmProjects\\GP\\compNet_noblazer.h5'
ONet=load_model(model_path)


doc={}
doc['path']=query_path
doc['labels']=[predict_piece(ONet,query_path)]
